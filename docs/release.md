...
| `v0.6.0` | `0.6.0` | ✅ passes |
| `0.6.0` | `0.6.0` | ✅ passes |
| `v0.6.0a1` | `0.6.0` | ❌ fails — cannot publish a pre-release tag from a stable package version |
| `v0.6.1` | `0.6.0` | ❌ fails — tag and package versions differ |

To publish a pre-release (e.g. for a dry-run on a real tag), bump the package version
to the same pre-release version first:

```bash
# pyproject.toml: version = "0.6.1a1"
git tag -a v0.6.1a1 -m "Pre-release v0.6.1a1"
git push origin v0.6.1a1
```
